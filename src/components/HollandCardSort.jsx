...
export default function HollandCardSort({ user, onCompleteHolland, onBackToOverview }) {
  ...
  return (
    <div className="max-w-3xl mx-auto py-6 space-y-6">
      ...
      {/* Top Header */}
      <div className="bg-white dark:bg-slate-900 rounded-2xl p-5 border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-3">
        <div>
        ...
        <div className="flex items-center space-x-3 text-xs font-semibold">
          <span className="px-3 py-1.5 bg-purple-50 dark:bg-purple-950 text-purple-700 dark:text-purple-300 rounded-full border border-purple-200 dark:border-purple-800">
      ...
      <div className="relative">
        <div className="bg-white dark:bg-slate-900 rounded-3xl p-8 sm:p-12 border-2 border-purple-200 dark:border-purple-900/60 shadow-2xl space-y-6 text-center transition-all transform hover:scale-[1.01]">
          ...
          <div className="flex flex-wrap items-center justify-center gap-2 pt-2">
            {currentCard.tags.map((tag, idx) => (
              <span key={idx} className="px-3 py-1 bg-purple-50 dark:bg-purple-950/60 text-purple-700 dark:text-purple-300 text-xs font-semibold rounded-lg border border-purple-200 dark:border-purple-900">
                #{tag}
              </span>
            ))}
          </div>
          ...
          {/* CARD ACTION BUTTONS */}
          <div className="pt-6 grid grid-cols-1 sm:grid-cols-3 gap-3 max-w-xl mx-auto">
            
            {/* LIKE */}
            <button
              onClick={() => handleChoice('like')}
              className={`p-4 rounded-2xl border-2 font-extrabold text-sm transition-all flex flex-col items-center justify-center space-y-1 ${
                currentChoice === 'like'
                  ? 'border-emerald-500 bg-emerald-600 text-white shadow-lg shadow-emerald-600/30 scale-105'
                  : 'border-emerald-200 dark:border-emerald-900/60 bg-emerald-50/50 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-300 hover:bg-emerald-600 hover:text-white'
              }`}
            >
            ...
            <button
              ...
              className={`p-4 rounded-2xl border-2 font-extrabold text-sm transition-all flex flex-col items-center justify-center space-y-1 ${
                currentChoice === 'unsure'
                  ...
                  : 'border-amber-200 dark:border-amber-900/60 bg-amber-50/50 dark:bg-amber-950/40 text-amber-700 dark:text-amber-300 hover:bg-amber-500 hover:text-white'
              }`}
            >
            ...
            <button
              ...
              className={`p-4 rounded-2xl border-2 font-extrabold text-sm transition-all flex flex-col items-center justify-center space-y-1 ${
                currentChoice === 'dislike'
                  ? 'border-rose-500 bg-rose-600 text-white shadow-lg shadow-rose-600/30 scale-105'
                  : 'border-rose-200 dark:border-rose-900/60 bg-rose-50/50 dark:bg-rose-950/40 text-rose-700 dark:text-rose-300 hover:bg-rose-600 hover:text-white'
              }`}
            >
      ...
      <div className="flex items-center justify-between pt-2">
        ...
        {currentIndex === HOLLAND_CARDS.length - 1 ? (
          <button
            ...
            className="px-8 py-3.5 bg-gradient-to-r from-purple-600 via-pink-600 to-amber-500 hover:from-purple-500 hover:to-amber-400 text-white font-black rounded-xl shadow-lg shadow-purple-500/30 transition-all hover:scale-105 active:scale-95 flex items-center space-x-2 text-base"
          >
          ...
          <button
            ...
            className="px-6 py-3 rounded-xl font-bold text-sm bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-200 hover:bg-slate-300 flex items-center space-x-2"
          >
