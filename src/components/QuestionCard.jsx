      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {question.options.map((opt) => {
          ...
          return (
            <div
              key={opt.id}
              className={`p-4 sm:p-5 rounded-2xl border-2 transition-all relative flex flex-col justify-between ${
                isMost
                  ? 'border-emerald-500 bg-emerald-50/70 dark:bg-emerald-950/40 ring-2 ring-emerald-500/20 shadow-md'
                  : isLeast
                  ? 'border-rose-500 bg-rose-50/70 dark:bg-rose-950/40 ring-2 ring-rose-500/20 shadow-md'
            ...
            >
              ...
              {/* Touch-Friendly Selection Control Buttons (Min 44px height on mobile) */}
